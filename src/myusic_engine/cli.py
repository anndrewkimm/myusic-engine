...
def _parser() -> argparse.ArgumentParser:
    ...
    external_parser = subparsers.add_parser(
        "map-musicbrainz",
        help="Map history metadata to MusicBrainz through a cached public mapper.",
    )
    ...
    external_parser.add_argument(
        "--limit",
        type=int,
        help="Optionally process only the highest-play-count tracks",
    )
    external_parser.add_argument(
        "--offline",
        action="store_true",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.command == "map-musicbrainz":
        ...
        try:
            ...
            def mapping_progress(processed: int, total: int) -> None:
                if processed == total or processed % 25 == 0:
                    print(f"Mapped {processed}/{total} history tracks...", flush=True)
        ...
        except (IdentityResolutionError, ProviderError, OSError) as exc:
        ...
        print(
            f"Exact MBIDs cover {external_result.report.exact_play_rate:.1%} of processed plays "
            f"and {external_result.report.exact_ms_played_rate:.1%} of listening time."
        )
